...
cramers_v <- function(x_name, y_name){
  x <- mpi_data_15[[x_name]]
  y <- mpi_data_15[[y_name]]
  t1 <- table(x,y)
  n <- nrow(mpi_data_15)
...
calc_redundancy <- function(x_name, y_name){
  ...
  y <- mpi_data_15[[y_name]]
  t1 = table(x,y)
  redundancy = (t1[2,2]/nrow(mpi_data_15))/
    (min(mean(x, na.rm = T),
         mean(y, na.rm = T),
         mean(x[mpi_data_15$mpi_poor == 1], na.rm =T),
         mean(y[mpi_data_15$mpi_poor == 1], na.rm =T)
    ))
  redundancy
}
...
MCA on Kentucky Data

```{r}
mpi_data_15 <- mpi_data_15 %>% ungroup()
...
```{r}
res <- MCA(mpi_mca[,-1], row.w = mpi_mca$PERWT)
```
...
```{r}
# df_boot <- mpi_data_15 %>% select(mpi_poor, PERWT)
# 
# df_length <- nrow(mpi_data_15)
# 
# nboot <- 1000
