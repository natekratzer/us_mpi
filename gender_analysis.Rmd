...
gender_df_per_con <- gender_df %>%
  filter(SEX == 1 | SEX == 2) %>%
  dplyr::select(SEX, YEAR, 'Adj. Headcount' = adj_hc, Headcount = headcount, Intensity = intensity, Income = per_con_inc, Employment = per_con_emp, Health = per_con_hlth, Disability = per_con_dis, Education = per_con_edu, Overcrowding = per_con_crowd, 'Housing Cost' = per_con_hcost, Internet = per_con_int, Language = per_con_lng) %>%
  gather(key = "Indicator", value = "dec_percent", -YEAR, -SEX) %>%
  mutate(Percent = dec_percent * 100)
```

```{r}
p <- ggplot(gender_df_per_con, aes(x = YEAR, y = Percent, group = SEX, linetype = SEX)) + geom_line() + facet_wrap(~Indicator) + 
  scale_x_continuous(labels = function(x) paste0("'", substr(x, 3, 4))) +
  labs(title = "Percent Contribution of Indicators by Gender", x = "Year", y = "Percent") + 
  theme_bw() + 
