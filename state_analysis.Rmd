...
png("fig_5_6_per_con_hlth_medicaid.png")
...
```{r}
png("fig_5_3_state_mpi_time.png")
...
```{r}
png("fig_5_5_per_con_hlth_medicaid_state.png")
ggplot(state_df, aes(YEAR, per_con_hlth *100, linetype = as.factor(Medicaid_Expansion))) +
  ...
  #theme(axis.text = element_blank(), axis.ticks = element_blank()) +
  facet_geo(~st_abr) +
  labs(title = "Figure 5.5 Percent Contribution of Health Insurance to MPI in U.S. states from 2013 to 2016", x = "Year", y = "Adjusted Headcount",
...
png("fig_5_4_per_con_state_bar_chart.png")
...
png("fig_5_1_state_choro.png")
plt_map <- state_choropleth(plt_map_df) + labs(title = "Figure 5.1 Adjusted Headcount, 2016") + theme(plot.title = element_text(hjust = 0.5))
dev.off()
```
...
p <- p + labs(title = "Figure 5.2 Income Poverty and Adjusted Headcount, 2016 ", y = "Adjusted Headcount", x = "Income Poverty") + theme_tufte()
```
...
ky_per_con_df <- state_df %>% 
  filter(STATEFIP == 21) %>%
  dplyr::select(YEAR, Income = per_con_inc, Employment = per_con_emp, 'Health Insurance' = per_con_hlth, Disability = per_con_dis, 'HS Degree' = per_con_edu, Overcrowding = per_con_crowd, 'Housing Cost' = per_con_hcost, Internet = per_con_int, Language = per_con_lng) %>%
  gather(key = "Indicator", value = "dec_percent", -YEAR) %>%
  mutate(Percent = dec_percent * 100)
```

```{r}
png("fig_5_7_per_con_ky_time.png")
