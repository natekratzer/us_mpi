...
ky_per_con_df <- state_df %>% 
  filter(STATEFIP == 21) %>%
  dplyr::select(YEAR, Income = per_con_inc, Employment = per_con_emp, Health = per_con_hlth, Disability = per_con_dis, Education = per_con_edu, Overcrowding = per_con_crowd, 'Housing Cost' = per_con_hcost, Internet = per_con_int, Language = per_con_lng) %>%
  gather(key = "Indicator", value = "dec_percent", -YEAR) %>%
  mutate(Percent = dec_percent * 100)
```

```{r}
p <- ggplot(ky_per_con_df, aes(x = YEAR, y = Percent)) + geom_line() + facet_wrap(~Indicator) + 
  labs(title = "Percent Contribution of Indicators in Kentucky", x = "Year", y = "Percent") + theme_bw()
```
